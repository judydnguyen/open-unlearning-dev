...
class LatentRMU(GradDiff):
    def __init__(
        ...
        **kwargs,
        ...
        self.forget_ce_weight = forget_ce_weight
        self._phase = 1
        self._phase2_step = 0
    ...
    def _enable_nonreentrant_gc(self):
        # use_reentrant=True (PyTorch default) runs each layer's forward inside
        # torch.no_grad(), so forward hooks in forward_with_cache capture
        # detached tensors — the loss has no grad_fn and backward() crashes.
        # use_reentrant=False runs the forward normally; hooks see grad-tracked
        # tensors. Must be set before Phase 2's super().train() so HF Trainer
        # does not override it back to the default.
    ...
    def train(self, resume_from_checkpoint=None, **kwargs):
        ...
        # Switch GC to use_reentrant=False before calling super().train() so
        # that forward hooks in forward_with_cache capture grad-tracked tensors.
        self._phase = 2
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        if self._phase == 1:
            ...
            # Wrap the forget forward in no_sync() so DDP doesn't start a gradient
            # reduction cycle before the retain forward. Both forwards accumulate
            # gradients locally; the single loss.backward() allreduces everything.
            with model.no_sync():
            ...
            # forget_outputs is already computed above at no extra cost.
            # Gradient ascent on the CE loss directly pushes output token probs
            # away from the memorized text, attacking verbatim recall.
            forget_ce = forget_outputs.loss if forget_outputs is not None else torch.tensor(0.0)
            loss = (
                self.gamma * warmup_coeff * forget_loss
                - self.forget_ce_weight * forget_ce
                + self.alpha * retain_loss
            )

            self.log({
                "train/forget_loss": forget_loss.item(),
                "train/forget_ce": forget_ce.item(),
