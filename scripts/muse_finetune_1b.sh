...
# Earlier recipe (lr=2e-5 / 5 ep / effective batch=32) underfit hard:
# loss 2.76 → 2.36 over 5 ep, VerbMem ROUGE = 0.16 (target should ~1.0).
# Smaller models tolerate higher LR and need more passes — bumping both.
per_device_train_batch_size=4
gradient_accumulation_steps=2   # effective batch = 4 * 2 * 2 GPUs = 16 (more updates)
learning_rate=5e-5              # 2.5x previous
num_train_epochs=10             # matches the 13B reference recipe
